...
public class BlurCore : MonoBehaviour
{
    ...
    protected void CheckForErrorsAndResolution()
    {
        ...
        texWidthVisibleSize = 100;
        texHeightVisibleSize = 100;
        checkForKernels();
    ...
    protected void warmUpTextures()
    {
        ...
        texWidthVisibleSize = 100; //Mathf.RoundToInt(thisCamera.pixelWidth * screenScaling);
        texHeightVisibleSize = 100; //Mathf.RoundToInt(thisCamera.pixelHeight * screenScaling);
    ...
    protected void DisptachWithSource(ref Texture2D source, ref RenderTexture destination)
    {
        if (!init)
            return;
    ...
    protected void DisptachWithSource(ref Texture2D source, ref RenderTexture destination, Material postProcessMat)
    {
        if (!init)
            return;
        int horizontalBlurDisX = Mathf.CeilToInt(((float)texWidthVisibleSize / (float)gpuMemoryBlockSizeBlur)); // it is here becouse res of window can change
